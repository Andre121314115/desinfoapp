...
// =========================
// Estado del sistema (Gemini + ML local)
// =========================
async function updateSystemStatus() {
  const statusGemini = $("#statusGemini");
  ...
  try {
    ...
    if (statusGemini) {
      if (json.ok) {
        statusGemini.textContent = "🔎 Verificador en línea (Gemini)";
...
async function showAnalysisResults(resultData, analysisId = null) {
  ...
  const score = final.score ?? gemini.score ?? 50;
  const verdict = final.verdict || gemini.verdict || "no_verificable";
    ...
    <div class="explanations-section">
      ...
      <div class="explanation-card">
        ...
        <div id="detailedExplanation" class="explanation-detailed" style="display: none;">
          ${explanations.detailed || (rationale || "No hay explicación detallada disponible.")}
        </div>
      </div>
  ...
  // Inicializar funcionalidades de explicaciones
  initExplanationFeatures(explanations);

  // Inicializar feedback solo si tenemos un id
  if (analysisId) {
...
function initExplanationFeatures(explanations) {
  ...
  $("#btnShareExplanation")?.addEventListener('click', function () {
    ...
    if (navigator.share) {
      ...
      alert('La función de compartir no está disponible en este dispositivo. Usa "Copiar explicación".');
    }
  });
}
...
function initFeedbackFeatures(analysisId, currentScore, currentVerdict) {
  ...
  if (!btnOk || !btnFix || !form || !inputScore || !selectVerdict || !btnSubmit) return;

  // Usuario confirma que está correcto
...
function renderMetrics(items) {
  ...
  const counts = items.reduce((acc, x) => {
    ...
    if (v.startsWith("cre") || v.startsWith("rea") || v.startsWith("ver")) acc.c++;
